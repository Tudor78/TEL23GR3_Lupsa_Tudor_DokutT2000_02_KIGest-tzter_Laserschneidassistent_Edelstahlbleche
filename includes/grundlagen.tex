...
In der Produktion kommen vor allem Faser- und Scheibenlaser zum Einsatz. CO\textsubscript{2}-Laser werden seltener verwendet. Faser- und Scheibenlaser erlauben hohe Energiedichten im Fokus und damit hohe Schnittgeschwindigkeiten, besonders bei dünnen bis mittleren Blechdicken. Die Wahl des Prozessgases beeinflusst Schnittqualität und Kantenverfärbung. Beim Schneiden mit Stickstoff bildet sich kaum Oxid an der Schnittkante, die Kante bleibt hell und metallisch. Das ist vorteilhaft, weil in der Regel weniger Nacharbeit erforderlich ist, wie z.B dem Nachbürsten der Kante, so dass wieder eine metalische Oberfläche zu sehen ist. Beim Schneiden mit Sauerstoff reagiert das Material mit dem Gas und somit entsteht eine dunkle Oxidschicht, die die Kante verfärbt. Der Prozess kann zwar schneller sein, führt aber demanch zu zusätzlicher Nacharbeit, wenn eine helle und einwandfreie Kante gefordert ist.
...
Die Abbildung~\ref{fig:conv-schema} zeigt die obig beschriebene Faltung mit einem $3{\times}3$-Kernel, welcher über ein Eingabebild läuft und eine Merkmalskarte erzeugt. Die Faltung des Kernels mit der Eingabe ergibt für die position \((0,0)\) den Wert \(y_{0,0}=16\).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\linewidth]{conv_demo.png}
  \caption[Schematischer Aufbau einer Faltung]{Schematischer Aufbau einer Faltung mit einem $3{\times}3$-Kernel, der über ein Eingabebild läuft und eine Merkmalskarte erzeugt \parencite{IBM_CNN_Topic}.}
...
Die ReLU-Aktivierung ist eine üblich genutzte Aktiviewrungsfunktion. Die Abbildung ~\ref{fig:ReLU} zeigt den Graphen der ReLU-Aktivierungsfunktion \(\phi(x)=\max(0,x)\). In dem Graphen wird ersichtlich, dass alle negativen Eingabewerte auf null gesetzt werden und alle positiven Werte unverändert bleiben. 

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.48\linewidth]{relu.png}
  \caption[ReLu Graph]{Graph der ReLU-Aktivierungsfunktion \(\phi(x)=\max(0,x)\) \parencite{FischerPochwyt_NeuronaleNetze_2017}.}
  \label{fig:ReLU}
\end{figure}
...
Die Abbildung~\ref{fig:maxpooling} zeigt den schematischen Aufbau eines Max-Pooling mit einem $2{\times}2$-Fenster und Schrittweite 2, der die räumliche Auflösung halbiert. Das selbe Prinzip der Abbildung gilt auch für Average-Pooling und Min-Pooling, nur dass dort der Durchschnitt bzw. der kleinste Wert im Fenster genommen wird \parencite{FischerPochwyt_NeuronaleNetze_2017}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.48\linewidth]{maxpool.jpeg}
  \caption[MaxPooling Schaubild]{Schematischer Aufbau eines Max-Pooling mit einem $2{\times}2$-Fenster und Schrittweite 2, der die räumliche Auflösung halbiert \parencite{FischerPochwyt_NeuronaleNetze_2017}.}
